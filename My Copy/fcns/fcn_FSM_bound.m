...
% In summary, the FSM logic controls the timing and transitions between 
% different states based on the elapsed time. Each state represents a 
% specific phase of the system's behavior, and the FSM ensures the system 
% follows the desired state sequence.

t = t_(1);
s = (t - Ta) / (Tb - Ta);
if (FSM == 1) && (s >= 1)
    % update time, FSM state and feet placement
    Ta = Tb;
    Tb = Tb + Tair;
    FSM = FSM + 1;
...
if FSM_(ii) == 1                 % 1 - front stance
    Fz_d = polyval_bz(p.Fz_co,s_(ii));  % GRF
    dz_d = polyval_bz(p.dz_co,s_(ii));  % Vertical position
    z_d = polyval_bz(p.z_co,s_(ii));    % Vertical velocity
    
    tau_d = polyval_bz(p.tau_co,s_(ii)); % torque
    dth_d = polyval_bz(p.dth_co,s_(ii)); % angular velocity
    th_d = polyval_bz(p.th_co,s_(ii));   % pitch angle
    R_d = expm(hatMap([0;th_d;0]));      % Orientation matrix
