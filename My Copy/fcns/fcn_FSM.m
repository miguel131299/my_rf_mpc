...
if isempty(FSM)
    FSM = zeros(4,1);
    Ta = zeros(4,1);    % Stance Start Time
    Tb = ones(4,1);     % Stance End Time / Swing Start Time
    pf_R_trans = Xt(idx_pf);
end

t = t_(1);      % current time
s = zeros(4,1); % normalized time within gait cycle
...
for i_leg = 1:4
    ...
    s(s>1) = 1;
    % --- FSM ---
    if FSM(i_leg) == 0          % initialization to stance

        if gait == -3           % backflip
            Ta(i_leg) = t;
            Tb([1,2]) = Ta([1,2]) + p.Tds*[1;1];  % where are Tds/Tbs defined?
            Tb([3,4]) = Ta([3,4]) + p.Tbs*[1;1];  % Mistake? Special Parameters for Backflip?
...
for i_leg = 1:4
    for ii = 2:p.predHorizon
        if t_(ii) <= Ta(i_leg)      % time before stance start time
            FSM_(i_leg,ii) = 1;     % set state to stance

        elseif (Ta(i_leg) < t_(ii)) && (t_(ii) < Tb(i_leg)) % time during stance
            FSM_(i_leg,ii) = FSM(i_leg);    % state stays the same (stance)

        elseif Ta(i_leg) + Tst + Tsw < t_(ii)   % completed full gait 
            FSM_(i_leg,ii) = FSM(i_leg);        % transition to next gait cycle

        else
            % Transition to next state
            if FSM(i_leg) == 1
                FSM_(i_leg,ii) = 2;     % stance -> swing
            else
                FSM_(i_leg,ii) = 1;     % swing -> stance
            end
        end
    end
end

% Handling special gaits

if gait == -1       % pose
    FSM_ = ones(size(FSM_));    % all legs forced to stance
elseif gait == -3   % backflip
    FSM(3:4) = 1;
    FSM_(:,3:4) = 1;            % legs 3 and 4 forced to stance
end
...
p = fcn_bound_ref_traj(p);      % why if not in bound gait? 
...
%position of hip joints in body frame
p_hip_b = [[L/2;W/2+d;0],[L/2;-W/2-d;0],[-L/2;W/2+d;0],[-L/2;-W/2-d;0]];
p_hip_R = R * p_hip_b;      % position of hip joints in global grame
ws = R * wb;                % angular velocity of swing leg in global frame
v_hip_R = repmat(dpc,[1,4]) + hatMap(ws) * p_hip_R; % velocity of hips in global frame

% capture point
% the capture point is a point of interest used in trajectoy planning and
% control

% p_cap = Tsw/2 * vd + sqrt(z0/g) * (vc - vd)
p_cap = zeros(2,4);         % capture point positions for each leg
vd = Xd(4:5,1);         % desired velocity
for i_leg = 1:4
    temp = 0.8 * Tst * vd + sqrt(p.z0/p.g) * (v_hip_R(1:2,i_leg) - vd);

    % clamp capture points
    temp(temp < -0.15) = -0.15;
    temp(temp > 0.15) = 0.15;
...
elseif p.gait == -2     % GOT (Gait Optimization Toolbox)?
    ...
    for i_leg = 1:4
        idx = 3*(i_leg-1) + (1:3);
        if FSM(i_leg) == 2
            % coefficients of polynomials (interpolation) between previous
            % foot positions "pf_r_trans" and the caputure point "p_cap"
            % along with normalized time "s"
