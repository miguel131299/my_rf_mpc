...
% Returns parameters corresponding to gait

p.predHorizon = 6;          % prediction horizon for MPC
p.simTimeStep = 1/200;      % time step used for simulation
p.Tmpc = 4/100;             % MPC prediction step time
p.gait = gait;              % type of gait
p.Umax = 50;                % maximum value for control inputs
p.decayRate = 1;            % decay rate for exponential weighting of past control actions
p.freq = 30;                % frequency of the gait
p.Rground = eye(3);         % covariance matrix for the ground frame orientation error
% Qf: final cost matrix for MPC Optimization problem
p.Qf = diag([1e5 2e5 3e5 5e2 1e3 150 1e3 1e4 800 40 40 10]);

% ---- gait ----
if gait == 1                % 1 - bound
    p.Tst = 0.1;            % stance time
    p.Tsw = 0.18;           % swing time
    p.predHorizon = 7;
    p.simTimeStep = 1/100;  
    p.Tmpc = 2/100;
    p.decayRate = 1;
    % Covariance Matrix for state estimation error
    p.R = diag(repmat([0.1 0.1 0.1]',[4,1])); 
    % Covariance Matrix fir MPC Optimization problem
...
%% Physical Parameters
p.mass = 5.5;
p.J = diag([0.026,0.112,0.075]); % moments of inertia
p.g = 9.81;     % gravitational acceleration
p.mu = 1;       % friction coefficient
p.z0 = 0.2;     % nominal COM height

% feet position in robots local coordinate frame
...
%% Swing phase 
p.Kp_sw = 300;  % Kp for swing phase (proportional gain/ sensitivity to error?)
