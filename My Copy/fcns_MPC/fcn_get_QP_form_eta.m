...
% INPUT:
% Xt: The current state vector.
% Ut: The current control input vector.
% Xd: The desired state trajectory.
% Ud: The desired control input trajectory.
% p: A structure containing various parameters for the QP problem.

% OUTPUT:
% H: The Hessian matrix.
% g: The gradient vector.
% Aineq: The inequality constraint matrix.
% bineq: The inequality constraint vector.
...
n_hor = p.predHorizon;      % prediction horizon
Umax = p.Umax;              % Max control input
decayRate = p.decayRate;    % decay rate

R = p.R;                    % Rotation Matrix
Q = p.Q;                    % Stage Cost Weighting Matrices
Qf = p.Qf;                  % Terminal Cost Weighting Matrices
...
% lb <= Fz <= ub
% bounds on vertical force
Fzd = Ud([3 6 9 12],:);
lb = -1 * Fzd;
ub = 2 * Fzd;

%% Matrices for QP
% Matrices initialzed with appropiate dimensions
H = zeros((nX + nU) * n_hor);
g = zeros(size(H,1),1);
...
for i_hor = 1:n_hor
    ...
    % H and q calculated based on weighting matrices Q and Qf and current
    % state and control input trajectories Xt, Ut, Xd, Ud. 
    % Components are weighted using the decay rate
    if i_hor == n_hor
    ...
    % The equality constraints (Aeq and beq) enforce the linear dynamics 
    % of the robot
    if i_hor == 1
    ...
    % The inequality constraints (Aineq and bineq) impose bounds on 
    % the control input and other constraints.
